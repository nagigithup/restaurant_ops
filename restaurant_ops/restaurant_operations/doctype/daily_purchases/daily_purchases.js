...
frappe.ui.form.on('Daily Purchases', {
	onload: set_today,
	branch: apply_branch_defaults,
	refresh(frm) {
		apply_branch_defaults(frm); purchase_total(frm);
		if (!frm.is_new()) {
			frm.add_custom_button(__('Create Purchase Invoice'), () => call_action(frm, 'restaurant_ops.restaurant_operations.doctype.daily_purchases.daily_purchases.create_purchase_invoice')).addClass('btn-primary');
			frm.add_custom_button(__('Create Purchase Receipt'), () => call_action(frm, 'restaurant_ops.restaurant_operations.doctype.daily_purchases.daily_purchases.create_purchase_receipt'));
			frm.add_custom_button(__('View Purchase Documents'), () => { if (frm.doc.linked_purchase_invoice) frappe.set_route('Form', 'Purchase Invoice', frm.doc.linked_purchase_invoice); else if (frm.doc.linked_purchase_receipt) frappe.set_route('Form', 'Purchase Receipt', frm.doc.linked_purchase_receipt); });
		}
	}
});
